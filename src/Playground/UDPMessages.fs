...
let pingLocal (sender: Agent<IPEndPoint * Message>) port =
  sender <-- (IPEndPoint (IPAddress.Parse "127.0.0.1", port), Ping)

let ping192 (sender: Agent<IPEndPoint * Message>) port =
  sender <-- (IPEndPoint (IPAddress.Parse "192.168.0.14", port), Ping)
